...
def sum[T <: SqlSingleConstType | Null](query: Expr[T]) = AggFunctionExpr[T]("SUM", List(query))

def avg[T <: SqlSingleConstType | Null](query: Expr[T]) = AggFunctionExpr[T]("AVG", List(query))
